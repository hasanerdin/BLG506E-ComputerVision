...
def compute_distances_two_loops(x_train, x_test):
  ...
  for train_data_idx in range(num_train):
    train_data_vector = x_train[train_data_idx].flatten()
    for test_data_idx in range(num_test):
        test_data_vector = x_test[test_data_idx].flatten()
        dist = (test_data_vector - train_data_vector).pow(2).sum().sqrt()
        dists[train_data_idx, test_data_idx] = dist
...
def compute_distances_one_loop(x_train, x_test):
  ...
  test_vector = x_test.flatten(start_dim=1)
  for train_data_idx in range(num_train):
    train_data_vector = x_train[train_data_idx].flatten()
    dist = (train_data_vector - test_vector).pow(2).sum(dim=1).sqrt()
    dists[train_data_idx] = dist
...
def compute_distances_no_loops(x_train, x_test):
  ...
  train_vector = x_train.flatten(start_dim=1)
  test_vector = x_test.flatten(start_dim=1)
  
  sum_square_train = train_vector.pow(2).sum(dim=1)[:, None]
  sum_square_test = test_vector.pow(2).sum(dim=1)
  multiplication = train_vector.matmul(test_vector.T)
    
  dists = (sum_square_train + sum_square_test - 2 * multiplication).sqrt()
...
def predict_labels(dists, y_train, k=1):
  ...
  _, topk_indices = torch.topk(dists.T, k, largest=False)
  topk_labels, _ = y_train[topk_indices].sort()
  y_pred, _ = topk_labels.mode(dim=1)
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  x_train_folds = x_train.tensor_split(num_folds)
  y_train_folds = y_train.tensor_split(num_folds)
  ...
  for k in k_choices:
    for fold_index in range(num_folds):
        x_validation = x_train_folds[fold_index]
        y_validation = y_train_folds[fold_index]
        
        x_rest_folds = x_train_folds[:fold_index] + x_train_folds[fold_index+1:] 
        x_rest = torch.cat(x_rest_folds)
        y_rest_folds = y_train_folds[:fold_index] + y_train_folds[fold_index+1:] 
        y_rest = torch.cat(y_rest_folds)
        
        classifier = KnnClassifier(x_rest, y_rest)
        accuracy = classifier.check_accuracy(x_validation, y_validation, k)
